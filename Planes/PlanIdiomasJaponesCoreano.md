...
- [ ] Definir nombres y regiones nativas: `日本語` / `日本` y `한국어` / `대한민국`
- [ ] Agregar los imports y entradas controladas de `ja-JP` y `ko-KR` en `src/i18n/index.js`
- [ ] Agregar las rutas controladas `JaJP/Index.js` y `KoKR/Index.js` en `Scripts/ValidarIdiomas.js`
...
- [ ] Probar los glifos sin conexión para confirmar que Android usa una fuente local de respaldo
- [ ] Definir una pila de fuentes de sistema compatible solo si la prueba detecta caracteres ausentes o pesos inconsistentes
...
- [ ] Ejecutar una simulación controlada de `release-notas-de-parche` y comprobar notas para `ja-JP` y `ko-KR`
...
- [ ] Ampliar `Scripts/ProbarIdiomas.js` con `ja`, `ja-JP`, `ko` y `ko-KR`; la prueba de persistencia recorrerá automáticamente todos los idiomas habilitados
